...
import { useDb, getUserByUsername, type User } from "../../lib/db";
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const u = await getUserByUsername(db, "alice");
  ...
  const usernameDisplay = user ? `@${user.username}` : loading ? "Loading…" : "—";
      ...
      <View style={styles.header}>
        ...
        <View style={styles.infoRow}>
          ...
          <View
            ...
            testID="createdBox"
            accessible
            accessibilityLabel="Account created"
          >
            <Text style={styles.infoLabel}>Joined</Text>
            <Text style={styles.infoValue}>{joinedDisplay}</Text>
          </View>
        </View>
        ...
        {Array.from({ length: 8 }).map((_, i) => (
          <View key={i} style={styles.activityCard}>
            ...
            <Text numberOfLines={2} style={styles.activitySubtitle}>
              This is where activity details will be rendered
            </Text>
          </View>
        ))}
      </ScrollView>
